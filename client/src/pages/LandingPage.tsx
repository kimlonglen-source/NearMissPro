  ...
  const groups: { heading: string; items: { q: string; a: string }[] }[] = [
    {
      ...
      items: [
        ...
        { q: 'What patient information do you collect?', a: 'None. The recording form has no patient fields, and the notes box blocks NHI numbers, dates of birth, phone numbers and names before anything is saved. There\'s no patient record to expose — even in a worst-case breach.' },
        { q: 'Does the AI see our data, and could it leak?', a: 'The AI only ever receives the near-miss details (drug, error type, contributing factors) to write its suggestion — no patient identity, because none is collected. Data sent to the AI is not used to train it, and you can turn AI suggestions off entirely and still use everything else.' },
        { q: 'Where is our data stored, and who can see it?', a: 'In a secure database hosted in Sydney, Australia — encrypted in transit and at rest. Each pharmacy can only ever see its own data, and we never sell it.' },
    ...
    },
    {
      ...
      items: [
        ...
        { q: 'Can records be changed or deleted after the fact?', a: 'Every action — a decision, an edit, a void, a restore, a report sign-off — is written to a timestamped audit log you can\'t quietly overwrite. Once a report is signed off, the near misses on it are locked; changing anything afterwards has to be done openly and is recorded. That tamper-evidence shows an inspector the record is trustworthy.' },
        { q: 'How long do you keep our data, who owns it, and can we get it out?', a: 'Your pharmacy owns its data. We keep it while your account is active, then 12 months after cancellation in case you reactivate, then delete it — or sooner on request. The database is backed up daily, and Settings has a one-click export of everything (near misses, reports and the full audit log) as a single file. Because near-miss records can count as workplace records under the Health and Safety at Work Act, we recommend exporting periodically so you always hold your own copy for as long as your practice requires.' },
      ],
    },
    {
      ...
      items: [
        ...
        { q: 'What about a dispensing error that reached the patient?', a: 'That\'s not a near miss. The first question on the form catches it and points you back to your pharmacy\'s dispensing-error process — for example the Pharmacy Council, the Health & Disability Commissioner, or your indemnity provider. This tool is only for events caught before they reached the patient.' },
        { q: 'Does this help with Pharmacy Council CQI expectations?', a: 'It\'s built around the continuous-improvement cycle — capture, review, act, and check whether last month\'s changes worked — and keeps a full timestamped audit trail, so your review work is documented as you go.' },
      ],
    },
    {
      ...
      items: [
        ...
        { q: 'What happens when the trial ends?', a: 'We email you before it ends. Nothing auto-charges — there\'s no card on file during the trial — so you\'re never billed by surprise.' },
        { q: 'Can I cancel any time?', a: 'Yes, any time. And you can export all your data whenever you like, so nothing is ever locked in.' },
      ],
    },
  ];
