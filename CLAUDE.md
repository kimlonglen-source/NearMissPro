...
- Founder credentials: in localhost dev (no env vars set) the password falls back to `founder123` and MFA accepts any 6 digits. In any deployed environment, BOTH `FOUNDER_PASSWORD` and `FOUNDER_TOTP_SECRET` must be set or the founder login is essentially open — the dev fallbacks are an intentional convenience, not a production posture. Generate a strong pair with `npm -w server run founder-setup` — it prints the env vars and the TOTP otpauth URI to add to Google Authenticator / 1Password / Authy. Founder email still comes from `FOUNDER_EMAIL`.
...
- **One email per pharmacy.** Captured at signup as "Dispensary email" and editable in Settings → Pharmacy. Stored in the legacy-named `manager_email` column (UI labels it "dispensary email" because that's what NZ pharmacies actually call this inbox — the column name is historical, not renamed to avoid a migration). Used as the destination for password-reset links and any future product emails. The separate "manager name" field was removed for simplicity; email greetings use only the pharmacy name.
- **Device verification on staff login.** Every browser gets a `nmp_device_id` UUID in localStorage. The server hashes it and looks up `trusted_devices` for the pharmacy. Unknown device → no token issued; instead an approval email is sent to the pharmacy email and the client shows "check email" instead of letting them in. Whoever owns the pharmacy email clicks the link, the device joins `trusted_devices`, and the original user re-submits the login to actually get in. Once approved, future logins from the same browser are friction-free. This replaces the abandoned IP allowlist as the "stop logins from home" control — softer (per-device, not per-network) but actually works for staff who legitimately move between devices.
- **Self-serve signup.** Pharmacies apply via `/signup` (full form: name, address, licence, manager name/email/phone, staffing size, source, notes). A `pending_approval` row is created in `pharmacies` with `password_hash` null. Founder gets an alert email + sees the application in `/admin → Pharmacies` under a "Pending review" section with Approve/Decline buttons. Approve → flips status to `trial`, generates a one-time `password_setup_tokens` row, emails the pharmacy a `/setup-password?token=…` link. Decline → flips to `declined`, sends a reason email if the founder provided one. The old founder-creates-the-pharmacy-by-hand path is gone; founders can no longer create pharmacies via the admin (only review applications). Login is blocked for `pending_approval` / `declined` / `suspended` states.
